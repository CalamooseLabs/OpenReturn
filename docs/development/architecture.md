...
The shared cursor matters: a second cursor mid-statement on the same connection makes a `commit()` on another raise *"SQL statements in progress"* during bulk ingest — so every concern uses the one cursor.

**Connection / cursor are thread-aware.** On the base `Database`, `connection` and `cursor` are **properties**: a concern delegates to the coordinator (`self._db`), and the coordinator routes to the *calling thread's* connection. By default everything uses the one main connection (CLI, ingest, single-threaded use). When the multi-threaded HTTP server calls `db.enable_threadlocal()` (in `cmd_serve`, file-backed DBs only — never `:memory:`), each **pool worker** thread lazily opens its **own** SQLite connection (opened `check_same_thread=False` so the main thread can close it at shutdown; WAL → concurrent readers; a generous `busy_timeout` for the occasional writer; a smaller per-connection cache since the shared mmap covers most reads). Because the server reuses a bounded pool of workers (see Server, below), the number of these connections is capped and they're reused across requests — not one per request. A slow query on one request no longer blocks every other client on a shared cursor. `close()` (main thread, after the pool has drained) closes the main connection and every per-thread connection.
...
| `db.scores` (`Score/` `ScoreDatabase`) | `get_factors()`, `get_model()`, `list_computed_models()`, `create_score()`, `finalize_score()`, `grade_factor()`, `replace_org_scores()` (batch), `all_eins()`, `get_score()`, `compare_scores()`, `list_score_history()`, `rank_leaderboard()`/`rank_org()`/`rank_org_dimensions()` (ranking — read the `org_score_latest` cache, live-window fallback), `rebuild_score_latest()` (rebuild that cache), `delete_filings_by_zip()` |
...
Wraps a `PooledHTTPServer` (an `HTTPServer` subclass that dispatches each request to a bounded `ThreadPoolExecutor`, `max_workers` set in `cmd_serve`). A bounded *reused* pool — rather than `ThreadingHTTPServer`'s thread-per-request — keeps the DB layer's per-thread connections capped at the pool size and reused across requests, instead of opening (and leaking) one connection per request. `server_close()` drains in-flight requests (`pool.shutdown(wait=True)`) before the coordinator closes connections. Wires routers to routes via `include_router()`.

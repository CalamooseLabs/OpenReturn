...
class TestCreateApiKey(TestApiKeyBase):
    def test_returns_tuple(self):
        result = self.db.keys.create_api_key("test-key")
        self.assertIsInstance(result, tuple)
        self.assertEqual(len(result), 2)

    def test_first_element_is_int(self):
        key_id, _ = self.db.keys.create_api_key("test-key")
        self.assertIsInstance(key_id, int)

    def test_second_element_is_str(self):
        _, raw = self.db.keys.create_api_key("test-key")
    ...
    def test_two_creates_return_different_keys(self):
        _, raw1 = self.db.keys.create_api_key("key-one")
        _, raw2 = self.db.keys.create_api_key("key-two")
        self.assertNotEqual(raw1, raw2)

    def test_two_creates_return_different_ids(self):
        id1, _ = self.db.keys.create_api_key("key-one")
        id2, _ = self.db.keys.create_api_key("key-two")
...
class TestValidateApiKey(TestApiKeyBase):
    def setUp(self):
        super().setUp()
        self.key_id, self.raw = self.db.keys.create_api_key("valid-key")

    def test_valid_key_returns_true(self):
        self.assertTrue(self.db.keys.validate_api_key(self.raw))

    def test_invalid_key_returns_false(self):
        self.assertFalse(self.db.keys.validate_api_key("not-a-real-key"))

    def test_empty_string_returns_false(self):
        self.assertFalse(self.db.keys.validate_api_key(""))

    def test_revoked_key_returns_false(self):
        self.db.keys.revoke_api_key(self.key_id)
        self.assertFalse(self.db.keys.validate_api_key(self.raw))
    ...
    def test_valid_use_updates_last_used_at(self):
        ...
        self.db.keys.validate_api_key(self.raw)
    ...
    def test_invalid_key_does_not_update_last_used_at(self):
        self.db.keys.validate_api_key("wrong-key")
        row = self.db.cursor.execute(
            "SELECT last_used_at FROM api_key WHERE key_id = ?", (self.key_id,)
        ).fetchone()[0]
    ...
    def test_second_call_returns_from_cache(self):
        first  = self.db.keys.validate_api_key(self.raw)
        second = self.db.keys.validate_api_key(self.raw)
        self.assertEqual(first, second)

    def test_invalid_key_cached_on_second_call(self):
        first  = self.db.keys.validate_api_key("bad-key")
        second = self.db.keys.validate_api_key("bad-key")
...
class TestListApiKeys(TestApiKeyBase):
    def test_empty_initially(self):
        self.assertEqual(self.db.keys.list_api_keys(), [])

    def test_returns_created_key(self):
        self.db.keys.create_api_key("my-key")
        keys = self.db.keys.list_api_keys()
        self.assertEqual(len(keys), 1)

    def test_has_required_fields(self):
        self.db.keys.create_api_key("my-key")
        key = self.db.keys.list_api_keys()[0]
    ...
    def test_includes_revoked_keys(self):
        key_id, _ = self.db.keys.create_api_key("my-key")
        self.db.keys.revoke_api_key(key_id)
        keys = self.db.keys.list_api_keys()
        self.assertEqual(len(keys), 1)
        self.assertFalse(keys[0]["active"])

    def test_ordered_by_key_id(self):
        self.db.keys.create_api_key("alpha")
        self.db.keys.create_api_key("beta")
        self.db.keys.create_api_key("gamma")
        keys = self.db.keys.list_api_keys()
        ids = [k["key_id"] for k in keys]
        self.assertEqual(ids, sorted(ids))

    def test_multiple_keys_all_returned(self):
        self.db.keys.create_api_key("a")
        self.db.keys.create_api_key("b")
        self.db.keys.create_api_key("c")
        self.assertEqual(len(self.db.keys.list_api_keys()), 3)


class TestRevokeApiKey(TestApiKeyBase):
    def test_returns_true_for_existing_key(self):
        key_id, _ = self.db.keys.create_api_key("my-key")
        self.assertTrue(self.db.keys.revoke_api_key(key_id))

    def test_returns_false_for_nonexistent_key(self):
        self.assertFalse(self.db.keys.revoke_api_key(99999))
    ...
    def test_validate_returns_false_after_revoke(self):
        key_id, raw = self.db.keys.create_api_key("my-key")
        self.assertTrue(self.db.keys.validate_api_key(raw))
        self.db.keys.revoke_api_key(key_id)
        self.assertFalse(self.db.keys.validate_api_key(raw))

    def test_does_not_affect_other_keys(self):
        id1, raw1 = self.db.keys.create_api_key("key-one")
        _id2, raw2 = self.db.keys.create_api_key("key-two")
        self.db.keys.revoke_api_key(id1)
        self.assertFalse(self.db.keys.validate_api_key(raw1))
        self.assertTrue(self.db.keys.validate_api_key(raw2))
